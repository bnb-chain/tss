...
var recoverCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		filenames := args
		clientChan := make(chan *client.TssClient)
		done := make(chan bool)
		for idx, filename := range filenames {
			go func(m string, i int, total int) {
				clientChan <- reloadTssClient(m, i, total)
			}(filename, idx, len(filenames))
		}

		go func() {
			for cc := range clientChan {
				go func(tssClient *client.TssClient) {
					tssClient.Start() // stuck until sign done
					done <- true
				}(cc)
			}
		}()
...
func reloadTssClient(filename string, idx int, total int) *client.TssClient {
	...
	checkPrerequisites(recoveredData, recoveredConfig, total)
	...
	c := client.NewRecoverTssClient(recoveredConfig, recoveredData)
	return c
}

func checkPrerequisites(data *keygen.LocalPartySaveData, config *common.TssConfig, total int) {
	if len(data.Ks) != config.Parties {
		common.Panic(errors.New("parties number not match"))
	}
	if config.Threshold > total {
		common.Panic(errors.New("signature thresold not match"))
	}
	if !data.Validate() {
...
func recoverHomeDir(config *common.TssConfig) {
	...
	config.Home = home + "/" + config.Moniker
